...
#! 25.5
#|
What changes are needed to make the cell id's a 2 element vector instead of a
three element list?

make-id, id-column, id-row, and id? would need to be changed to also reflect the
change in underlying implementation.
|#

;; Cell IDs

(define (make-id col row)
  (let ((id-vector (make-vector 2)))
    (vector-set! id-vector 0 col)
    (vector-set! id-vector 1 row)
    id-vector))
